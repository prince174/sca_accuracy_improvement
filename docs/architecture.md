...
Образы считаются недоверенными. Контейнер не создаётся и не запускается.
`docker image save` формирует архив слоёв, который анализатор читает напрямую с
учётом удалений overlay whiteout. Архив не распаковывается на хост. ZIP/JAR
читаются с ограничением размера.
