...
API key и timeout. Для on-premise меняется адаптер или конфигурация inference
endpoint. Форматы evidence, assessment и policy gate остаются прежними. Перед
переключением обе модели прогоняются на закреплённом наборе вручную размеченных
сборок и findings.
