...
сохраняется. При `with_llm=true` модуль `scoring.py` создаёт каталог исходных и
обнаруженных компонентов; `llm.py` получает оценки TP порциями до 40 записей.
Каждая запись должна получить ровно одну корректную оценку и ссылки на evidence.
Код включает в результат только **TP >70%**, добавляет прошедшие порог наблюдения
и очищает ссылки на исключённые компоненты. Все баллы и основания сохраняются
в `component-scores.json`, `assessment.json` и HTML-отчёте. Баллы некалиброванные;
валидатор проверяет контракт, но не доказывает истинность оценки.

`image.py` дополнительно извлекает статические JVM-ссылки. Они не являются
полным call graph или доказательством исполнения. После фильтра повторная сверка
и `enrich_sbom(add_observed=False)` добавляют аннотации; исключённые записи не
возвращаются обратно. При `with_llm=false` действует прежнее обогащение без
фильтрации исходных компонентов. Исторический `decisions.py` используется
в старых benchmarks; основной модельный поток описан в [политике](scoring-policy.md).
